...
func TestMultiParse(t *testing.T) {
	for _, test := range multiParseTests {
		template, err := Text("root").Parse(test.input)
		switch {
		...
		/* omitted - no access to non-exported fields
...
func TestMultiExecute(t *testing.T) {
	...
	template, err := Text("root").Parse(multiText1)
	if err != nil {
...
func TestParseFiles(t *testing.T) {
	_, err := ParseTextFiles("DOES NOT EXIST")
	if err == nil {
...
func TestParseGlob(t *testing.T) {
	_, err := ParseTextGlob("DOES NOT EXIST")
	if err == nil {
		t.Error("expected error for non-existent file; got none")
	}
	_, err = Text("error").ParseGlob("[x")
	if err == nil {
...
func TestParseFilesWithData(t *testing.T) {
	template, err := Text("root").ParseFiles("testdata/tmpl1.tmpl", "testdata/tmpl2.tmpl")
	if err != nil {
		t.Fatalf("error parsing files: %v", err)
	}
...
func TestParseGlobWithData(t *testing.T) {
	template, err := Text("root").ParseGlob("testdata/tmpl*.tmpl")
	if err != nil {
		t.Fatalf("error parsing files: %v", err)
	}
	testExecute(templateFileExecTests, template, t)
...
func TestClone_multi(t *testing.T) {
	// Create some templates and clone the root.
	root, err := Text("root").Parse(cloneText1)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAddParseTree_multi(t *testing.T) {
...
func TestRedefinition(t *testing.T) {
	var tmpl Template
	var err error
	if tmpl, err = Text("tmpl1").Parse(`{{define "test"}}foo{{end}}`); err != nil {
...
func TestText(t *testing.T) {
	...
	t1, _ := Text("test").Parse(`{{define "test"}}foo{{end}}`)
	t2 := t1.New("test")
...
func TestEmptyTemplate_multi(t *testing.T) {
	cases := []struct {
		defn []string
		in   string
		want string
...
func TestIssue19294(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		res, err := Text("title.xhtml").Parse(`{{template "xhtml" .}}`)
		if err != nil {
			t.Fatal(err)
		}
