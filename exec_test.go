...
var tVal = &T{
	...
	Tmpl:                 Must(Text("x").Parse("test template")), // "x" is the value of .X
}
...
func testExecute(execTests []execTest, template Template, t *testing.T) {
	...
	for _, test := range execTests {
		...
		var err error
		if template == nil {
			tmpl, err = Text(test.name).Funcs(funcs).Parse(test.input)
		} else {
...
func TestDelims(t *testing.T) {
	...
	for i := 0; i < len(delimPairs); i += 2 {
		...
		tmpl, err := Text("delims").Delims(left, right).Parse(text)
		if err != nil {
...
func TestExecError(t *testing.T) {
	tmpl, err := Text("top").Parse(execErrorText)
	if err != nil {
...
func TestTree(t *testing.T) {
	...
	tmpl, err := Text("root").Delims("(", ")").Parse(treeTemplate)
	if err != nil {
		t.Fatal("parse error:", err)
	}
	var b bytes.Buffer
...
func TestExecuteOnNewTemplate(t *testing.T) {
	...
	Text("Name").Templates()
	// This is issue 11379.
	/* new(Template) is pointer to interface, not interface. With a var newTemplate Template we crash - rightfully
...
func TestMessageForExecuteEmpty(t *testing.T) {
	...
	tests, err := Text("").Parse(testTemplates)
	if err != nil {
		t.Fatal(err)
	}
	_ = tests
	/* omitted we have no field Tree
...
func TestComparison(t *testing.T) {
	...
	for _, test := range cmpTests {
		...
		tmpl, err := Text("empty").Parse(text)
		if err != nil {
...
func TestExecuteGivesExecError(t *testing.T) {
	...
	tmpl, err = Text("X").Parse("hello, {{.X.Y}}")
	if err != nil {
		t.Fatal(err)
	}
...
func TestBlock(t *testing.T) {
	...
	tmpl, err := Text("outer").Parse(input)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMissingFieldOnNil(t *testing.T) {
	tmpl := Must(Text("tmpl").Parse("{{.MissingField}}"))
	var d *T
	...
	want := "can't evaluate field MissingField in type *template_test.T"
...
func TestMaxExecDepth(t *testing.T) {
	tmpl := Must(Text("tmpl").Parse(`{{template "tmpl" .}}`))
...
func TestAddrOfIndex(t *testing.T) {
	...
	for _, text := range texts {
		tmpl := Must(Text("tmpl").Parse(text))
		var buf bytes.Buffer
...
func TestInterfaceValues(t *testing.T) {
	...
	for _, tt := range tests {
		tmpl := Must(Text("tmpl").Parse(tt.text))
		var buf bytes.Buffer
