...
func TestEscape(t *testing.T) {
	data := struct {
		...
		M: &goodMarshaler{},
		Z: nil,
		W: data.HTML(`&iexcl;<b class="foo">Hello</b>, <textarea>O'World</textarea>!`),
	}
	pdata := &data
	...
	tests := []struct {
		...
		},
		{
			...
			`<button onclick='alert(1/ /* json: error calling MarshalJSON for type *template_test.badMarshaler: invalid character &#39;f&#39; looking for beginning of object key string */null in numbers)'>`,
		},
		{
			"jsMarshaler",
...
func TestEnsurePipelineContains(t *testing.T) {
	...
	for i, test := range tests {
		tmpl := Must(Text("test").Parse(test.input))
		_, _, _ = i, test, tmpl
		/* omit
...
func TestRedundantFuncs(t *testing.T) {
	inputs := []interface{}{
		"\x00\x01\x02\x03\x04\x05\x06\x07\x08\t\n\x0b\x0c\r\x0e\x0f" +
			...
			"\u00A0\u0100\u2028\u2029\ufeff\ufdec\ufffd\uffff\U0001D11E" +
			"&amp;%22\\",
		data.CSS(`a[href =~ "//example.com"]#foo`),
		data.HTML(`Hello, <b>World</b> &amp;tc!`),
		data.HTMLAttr(` dir="ltr"`),
		data.JS(`c && alert("Hello, World!");`),
		data.JSStr(`Hello, World & O'Reilly\x21`),
		data.URL(`greeting=H%69&addressee=(World)`),
	}
	_ = inputs
	/* omit
